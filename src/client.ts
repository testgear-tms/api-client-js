...
export class Client implements IClient {
  ...
  private readCliConfig(): Partial<ClientConfig> {
    ...
    let config = {};
    if (args['testgearConfig']) {
      config = this.readConfigFile(args['testgearConfig']);
    }
    return Client.mergeConfig(config, {
      url: args['testgearUrl'],
      privateToken: args['testgearPrivateToken'],
      projectId: args['testgearProjectId'],
      configurationId: args['testgearConfigurationId'],
      testRunId: args['testgearTestRunId'],
    });
  }

  private readEnvConfig(): Partial<ClientConfig> {
    let config = {};
    if (process.env['TESTGEAR_CONFIG_FILE']) {
      config = this.readConfigFile(process.env['TESTGEAR_CONFIG_FILE']);
    }
    return Client.mergeConfig(config, {
      url: process.env['TESTGEAR_URL'],
      privateToken: process.env['TESTGEAR_PRIVATE_TOKEN'],
      projectId: process.env['TESTGEAR_PROJECT_ID'],
      configurationId: process.env['TESTGEAR_CONFIGURATION_ID'],
      testRunId: process.env['TESTGEAR_TEST_RUN_ID'],
    });
  }
